...
export default function Chatbot() {
  ...
  return (
    <>
      ...
      {open && (
        <div className="fixed bottom-6 right-6 z-50 w-[400px] max-w-[calc(100vw-2rem)] bg-white dark:bg-slate-800 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-700 flex flex-col overflow-hidden animate-in slide-in-from-bottom-4 duration-300"
          style={{ height: '580px' }}
        >
              ...
              >
                <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${
                  msg.role === 'user'
                    ...
                    : 'bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-100'
                }`}>
                  {msg.role === 'user' ? (
                ...
                <div className={`max-w-[85%] group ${
                  msg.role === 'user'
                    ...
                    : 'bg-white dark:bg-slate-700 dark:text-slate-100 text-slate-800 rounded-2xl rounded-tl-md shadow-sm border border-slate-100 dark:border-slate-700'
                }`}>
              ...
              <div className="grid grid-cols-2 gap-2">
                {QUICK_ACTIONS.map((action) => (
                  <button
                    ...
                    className="flex items-center gap-2 px-3 py-2 bg-slate-50 dark:bg-slate-700 hover:bg-indigo-50 dark:hover:bg-slate-600 text-slate-700 dark:text-slate-100 hover:text-indigo-700 rounded-lg transition-colors text-left"
                  >
            ...
            >
              <div className="relative flex-1">
                <input
                  ...
                  disabled={loading}
                  className="w-full text-sm bg-slate-50 dark:bg-slate-700 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-3 pr-10 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-400 placeholder-slate-400 dark:placeholder-slate-400 disabled:opacity-60 transition-all text-slate-800 dark:text-slate-100"
                />
